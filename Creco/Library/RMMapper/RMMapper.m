...
+ (NSDictionary *)propertiesForClass:(Class)cls
{
    ...
    //Only find superclass once
    if ([cls superclass] != [NSObject class]) {
        properties = class_copyPropertyList([cls superclass], &outCount);
        for (i = 0; i < outCount; i++) {
            objc_property_t property = properties[i];
            const char *propName = property_getName(property);
            if(propName) {
...
+(id)populateObject:(id)obj fromDictionary:(NSDictionary *)dict exclude:(NSArray *)excludeArray {
    ...
    for (id key in dict) {
        ...
        if (properties.count > 0 && [properties objectForKey:key] == nil && ![key isEqualToString:@"description"]) {
