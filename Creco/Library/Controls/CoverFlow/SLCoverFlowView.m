...
- (void)reloadData {
    ...
    if (![[NSUserDefaults standardUserDefaults] boolForKey:@"isCalendar"]) {
        [self layoutSubviews];
    }
}
...
- (SLCoverView *)insertCoverViewAtIndex:(NSInteger)index frame:(CGRect)frame {
    SLCoverView *coverView = [self.parentView.dataSource coverFlowView:self.parentView coverViewAtIndex:index];
...
- (void)reloadData {
    _numberOfCoverViews = [self.dataSource numberOfCovers:self];
...
- (void) move:(int)index {
    CGPoint position = CGPointMake((280+10)*index, 0);
...
{
    if (self.delegate && [self.delegate respondsToSelector:@selector(coverFlowViewNowMovingIndex:)])
    {
        NSInteger index = [self nearByIndexOfScrollViewContentOffset:scrollView.contentOffset];
        if (index == -1) index = 0;
        [self.delegate coverFlowViewNowMovingIndex:index];
    }
}
...
- (void)scrollViewWillEndDragging:(UIScrollView *)scrollView
    ...
    if (_endDraggingVelocity.x == 0) {
        ...
        *targetContentOffset = [self nearByOffsetOfScrollViewContentOffset:_scrollView.contentOffset];
        ...
        // avoid boucing back
        NSInteger index = [self nearByIndexOfScrollViewContentOffset:nearByOffset];
        if (_endDraggingVelocity.x > 0) {
            if (nearByOffset.x < endOffsetX) {
                if (index < (_numberOfCoverViews-1)) {
                    ...
                    if (self.delegate && [self.delegate respondsToSelector:@selector(coverFlowViewMovedEndAtIndex:)])
                        [self.delegate coverFlowViewMovedEndAtIndex:index+1];
                }
            }
        } else {
            if (nearByOffset.x > endOffsetX) {
        ...
        if (!indexChanged) {
            if (self.delegate && [self.delegate respondsToSelector:@selector(coverFlowViewMovedEndAtIndex:)])
                [self.delegate coverFlowViewMovedEndAtIndex:index];
        }
