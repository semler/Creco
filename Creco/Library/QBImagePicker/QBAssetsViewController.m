...
- (void)viewDidLoad
{
    ...
    // 確定ボタン
    self.collectionView.frame = CGRectMake(self.collectionView.frame.origin.x, self.collectionView.frame.origin.y, self.collectionView.frame.size.width, self.collectionView.frame.size.height-49);
...
- (void)setUpToolbarItems
{
    ...
    [leftSpace setTitleTextAttributes:@{NSForegroundColorAttributeName:[ShareMethods colorFromHexRGB:[[NSUserDefaults standardUserDefaults] valueForKey:kAppTopicColor]]} forState:UIControlStateNormal];
    ...
    [assetsLibrary assetForURL:assetURL
                   resultBlock:^(ALAsset *asset) {
                       ...
                       if(asset){
                           ...
                           UIImage *orgImage = [UIImage imageWithCGImage:imgRef scale:assetRep.scale orientation:(UIImageOrientation)assetRep.orientation];
                           _image = orgImage;
